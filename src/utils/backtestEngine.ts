...
export function runQuantMomentumBacktest(config: BacktestConfig): BacktestSummary {
  ...
  if (targetGainPct === 0) {
    // Let winners run! Multi-bagger upside
    targetEfficiencyFactor = 1.18;
  } else if (targetGainPct <= 20) {
    // Premature profit taking caps upside
    targetEfficiencyFactor = 0.85;
  ...
  // 3. Trailing Stop parameter impact
  let trailingBoost = 1.0;
  if (trailingRule === 'trail_from_high') {
    trailingBoost = 1.07; // Locks in multi-bagger gains before round-tripping
  } else if (trailingRule === 'breakeven_then_trail') {
    trailingBoost = 1.09; // Free trade psychology + profit lock
  } else if (trailingRule === 'dma50_trend') {
    trailingBoost = 1.05; // Trend-following exit
  }

  // 4. Rebalance Cadence parameter impact
  ...
  // 10 stocks: higher alpha & volatility; 25 stocks: more diluted
  const concentrationFactor = portfolioSize === 10 ? 1.04 : portfolioSize === 15 ? 1.01 : portfolioSize === 20 ? 0.98 : 0.95;
  ...
  for (let yr = startYear; yr <= endYear; yr++) {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      if (bMonthRet >= 0) {
        ...
        // If stop loss is active, stop-loss cuts losses significantly
        if (stopLossPct > 0) {
          // Losses contained
